 ...
 *            					 
 * {function} [fnSuccess] a callback function which is called when the data has
 *            					 been successfully updated.
 *            
 * {function} [fnError] a callback function which is called when the request failed. The handler can have the parameter: oError which contains
 * additional error information.
 * 
 * {Boolean} [bMerge=false] trigger a MERGE request instead of a PUT request to perform a differential update
 * {String} [sETag] If specified, the If-Match-Header will be set to this Etag.
 * 
...
//ensure compatibility, check for old or new declaration of parameters
	if ((oParameters && oParameters instanceof sap.ui.model.Context) || arguments[3])
	{
		oContext = oParameters;
		fnSuccess = arguments [3];
		fnError = arguments [4];
		bMerge = arguments [5];
	} else if (oParameters &! oParameters instanceof sap.ui.model.Context) {
 ...
 * {Object} [oContext] If specified the sPath has to be relative to the path given with the context.
 * {function} [fnSuccess]  a callback function which is called when the data has
 *            					 been successfully retrieved. The handler can have the 
 *            	                 following parameters: oData and response.
 * {function} [fnError] a callback function which is called when the request failed. The handler can have the parameter: oError which contains
 ...
 *            				which was specified in the model constructor.
 * @param {Object} [oContext] If specified the sPath has to be is relative to the path given with the context. 
 * @param {Array} [aUrlParams] An Array of url parameters which will be concatenated to the read url
 *
 ...
 * @param {Object} [oParameters] optional parameter for additional information introduced in SAPUI5 1.9.1, can have an attribute [sETag]
 * carrying an ETag which can be used for concurrency control. If it is specified, it will be used in an If-Match-Header in the request to the server for this entry.    
 * @public
 */
