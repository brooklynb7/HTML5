...
sap.ui.core.Control.extend("sap.ui.ux3.NotificationBar.NotifierView", {
	...
	init : function() {
		this._oResBundle = sap.ui.getCore().getLibraryResourceBundle(
				"sap.ui.ux3");
	},
...
(function() {
	...
	var fnChildEventListener = function(oEvent) {
		...
		case "removed":
			...
			if (this.getMessageNotifier()
					&& this.getMessageNotifier().getId() === oNotifier.getId()) {
				...
				if (aSortMessages.length > 0) {
					...
					this._sSeverestMessageLevel = aSortMessages[iIndex]
							.getLevel();
				}
			}
				...
				if (oEvent.getParameter("type") === "removed") {
					...
					for ( var i = 0; i < aMessageViews.length; i++) {
						...
						if (oMessage.getId() === tmpMsgView._message.getId()) {
							...
							/*
							 * A normal invalidate on the NotifierView doesn't
							 * work here since the invalidate is delegated to
							 * the most upper parent -> the NotificationBar. If
							 * the NotificationBar is rendered it doesn't know
							 * something of the corresponding Callout. It would
							 * be possible to rerender the NotifierView but
							 * since the height of the Callout is reduced the
							 * Callout's position must be fixed as well. This
							 * happens also when the Callout is rerendered, so
							 * it saves some lines of codes to rerender the
							 * Callout itself.
							 */
							oCallout.rerender();
							break;
	...
	var fnRegisterNotifierToEvents = function(that, oNotifier) {
		oNotifier
				.attachEvent("_childControlCalling", that._eventListener, that);
	};
	...
	var fnDeregisterNotifierFromEvents = function(that, oNotifier) {
		oNotifier
				.detachEvent("_childControlCalling", that._eventListener, that);
	};
	...
	sap.ui.ux3.NotificationBar.prototype.addNotifier = function(oNotifier) {
		if (oNotifier) {
			var bSuppress = (this.getVisibleStatus() == sap.ui.ux3.NotificationBarStatus.None) ? true
					: false;
	...
	sap.ui.ux3.NotificationBar.prototype.insertNotifier = function(oNotifier,
			iIndex) {
		if (oNotifier) {
	 ...
	 */
	sap.ui.ux3.NotificationBar.prototype.setMessageNotifier = function(
			oMessageNotifier) {
	...
	sap.ui.ux3.NotificationBar.prototype.destroyMessageNotifier = function(
			oMsgNotifier) {
	...
	var fnSetSeverityForMessageNotifier = function(oThis, oMN) {
		if (oMN && oMN.hasItems()) {
			...
			var sKey = "NOTIBAR_MESSAGE_NOTIFIER_DESC_LEVEL_"
					+ sLvl.toUpperCase() + iCount === 1 ? "_SING" : "_PL";
	...
	var fnSetItemsDescription = function(oThis) {
		...
		for ( var i = 0; i < aNotifiers.length; i++) {
			...
			var sKey = "NOTIBAR_NOTIFIER_COUNT_TEXT_" + iCount === 1 ? "SING"
					: "PL";
	...
	sap.ui.ux3.NotificationBar.prototype.getHeightOfStatus = function(sStatus) {
		...
		if (sStatus == sap.ui.ux3.NotificationBarStatus.Min) {
			...
			if (iIndex != -1) {
				...
				var _iHeight = parseInt(this
						.getHeightOfStatus(sap.ui.ux3.NotificationBarStatus.Default));
				if (iHeight < _iHeight) {
					iHeight = _iHeight + 1;
				}
			} else {
