 ...
 * <li>{@link #getFixedColumnCount fixedColumnCount} : int (default: 0)</li></ul>
 * </li>
 * <li>Aggregations
 * <ul>
...
sap.ui.table.Table.prototype._initItemNavigation = function() {
	...
	if (this.getFixedColumnCount() == 0) {
		...
		var $scrollTable = this.$().find('.sapUiTableCtrlScroll');
		var $fixedTable = this.$().find('.sapUiTableCtrlFixed');
		for (var i = 0; i < this.getVisibleRowCount(); i++) {
			aItemDomRefs = aItemDomRefs.concat($fixedTable.find('tr[data-sap-ui-rowindex]:eq(' + i + ')').find('td[tabindex]').get());
			aItemDomRefs = aItemDomRefs.concat($scrollTable.find('tr[data-sap-ui-rowindex]:eq(' + i + ')').find('td[tabindex]').get());
		}
	}
...
sap.ui.table.Table.prototype._attachEvents = function() {
	...
	$this.find(".sapUiTableCtrlFixed > tbody > tr").hover(function() {
		...
		$this.find(".sapUiTableCtrlScroll > tbody > tr").filter(":nth-child(" + (iIndex + 1) + ")").addClass("sapUiTableRowHvr");
	}, function() {
		jQuery(this).removeClass("sapUiTableRowHvr");
	...
	$this.find(".sapUiTableCtrlScroll > tbody > tr").hover(function() {
		...
		$this.find(".sapUiTableRowHdr").filter(":nth-child(" + iIndex + ")").addClass("sapUiTableRowHvr");
		$this.find(".sapUiTableCtrlFixed > tbody > tr").filter(":nth-child(" + (iIndex + 1) + ")").addClass("sapUiTableRowHvr");
	}, function() {
		jQuery(this).removeClass("sapUiTableRowHvr");
		$this.find(".sapUiTableRowHdr").removeClass("sapUiTableRowHvr");
...
sap.ui.table.Table.prototype._updateSelection = function() {
	...
	$this.find(".sapUiTableRowHdr").each(function(iIndex, oElement) {
		var $jqTR1 = jQuery($this.find(".sapUiTableCtrlFixed > tbody > tr").get(iIndex + 1));
		var $jqTR2 = jQuery($this.find(".sapUiTableCtrlScroll > tbody > tr").get(iIndex + 1));
		...
		if (that.isIndexSelected(iFirstRow + iIndex)) {
			...
			if (bMultiSelection) {
				$jqDIV.attr("title", oResBundle.getText("TBL_ROW_DESELECT_MULTI")).
