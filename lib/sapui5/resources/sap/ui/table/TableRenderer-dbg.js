...
sap.ui.table.TableRenderer.renderTableControl = function(rm, oTable, bFixedTable) {
	...
	rm.write("<table");
	if (bFixedTable) {
		rm.writeAttribute("id", oTable.getId() + "-table-fixed");
	...
	//Firefox needs a defined width for the fixed table
	if (bFixedTable && jQuery.browser.mozilla) {
	...
	for (var row = 0, count = aRows.length; row < count; row++) {
		this.renderTableRow(rm, oTable, aRows[row], row, bFixedTable, iStartColumn, iEndColumn);
	}

	rm.write("</table>");
};

sap.ui.table.TableRenderer.renderTableRow = function(rm, oTable, oRow, iRowIndex, bFixedTable, iStartColumn, iEndColumn) {
