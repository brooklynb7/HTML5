...
sap.m.Select.prototype.onAfterRendering = function() {
	...
	// jQuery DOM reference to the label used to show the text from the current selected item
	this._$SltLabel = this._$SltCont.children(".sapMSltText");

	if (this._$SltNative.length) {	// do stuff if the select exist

		// register a listener to the select change event
		this._$SltNative.on("change.sapMSelect", jQuery.proxy(this._handleChange, this));

		this._$SltNative.css("font", this._$SltLabel.css("font"));

		if (this.getWidth() === "auto") {
			this._$SltLabel.width(this._$SltNative.width());
		}
	}
...
sap.m.Select.prototype._handleChange = function() {
	...
	this._$SltLabel.text(oItem.getText());
...
sap.m.Select.prototype._unbindChange = function() {
	if (this._$SltNative instanceof jQuery && this._$SltNative.length) {
		this._$SltNative.off("change.sapMSelect", this._handleChange);
	}
};
