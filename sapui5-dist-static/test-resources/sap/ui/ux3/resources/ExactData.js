	...
	atts: [ (function(){
				...
				for(var i=0; i<aTestData[0].attributes.length; i++){
					...
					if(typeof oCountry == "string"){
						...
						if(oCountry != "Canada") resCountry.subVals = false; /*subVals not set in case of 'Canada' to test the default behavior (arrow should appear initially and then should disappear on select)*/
					}else{
						resCountry = {name: oCountry.name};
						resCountry.atts = (function(){
												var atts = [];
...
exactTestData.initAttributes = function(oControl){
	var aData = aTestData;
	if(oControl._bTestDataInitialized) return;
...
exactTestData.initAttributesForQUnit = function(oControl, sIdPrefix){
	...
	function createAttr(sIdx, iSubAttCount) {
		var att = new sap.ui.ux3.ExactAttribute(sIdPrefix+"att"+sIdx, {text: ""+sIdx});
		for(var i=0; i<iSubAttCount; i++){
			att.addAttribute(new sap.ui.ux3.ExactAttribute(sIdPrefix+"att"+sIdx+"-"+(i+1), {text: sIdx+"-"+(i+1)}));
		}
		return att;
	}
